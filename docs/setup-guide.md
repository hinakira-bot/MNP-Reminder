...
```
https://discord.com/oauth2/authorize?client_id=1476408639043539044&permissions=2415921920&scope=bot+applications.commands
```

→ サーバーを選択 →「認証」を押す

> ※ Bot には「メッセージ送信」「埋め込みリンク」「ロール管理」の権限が必要です。
...
実践した人に自動でロール（名前の色変更など）を付けたい場合：

**① ロールを作成する**
サーバー設定 → ロール → 「ロールを作成」 → 名前を「実践者」などにして、色を設定 → 保存

**② Botのロールの位置を調整する**
サーバー設定 → ロール で、**「MNP実践トラッカー」のロールを「実践者」ロールより上にドラッグ**して移動

**③ Botにロール管理の権限を付ける**
サーバー設定 → ロール → 「MNP実践トラッカー」をクリック → 権限タブ → **「ロールの管理」をON** → 保存
...
> ⚠️ ②と③を忘れるとロールが付きません。必ず両方設定してください。
...
**Q. ボタンは何回でも押せますか？**
はい。実践するたびに押してください。連打防止のため、1分以内の連続クリックは無視されます。

**Q. `/setup` をもう一度実行したらどうなる？**
古いボタンが自動で削除され、新しいボタンに置き換わります。
...
**Q. ロールが付かない場合は？**
以下を確認してください：
1. `/set-practice-role @ロール名` を実行済みか
2. サーバー設定 → ロールで、Botのロールが「実践者」ロールより**上**にあるか
3. Botのロールの権限で「ロールの管理」が**ON**になっているか

**Q. ロールはいつ付きますか？**
実践完了ボタンを押した時に自動付与されます。
